...
namespace AaltoGames
{
	...
	void ControlPBP::init(int nSamples, int nSteps, int nStateDimensions, int nControlDimensions, const double *controlMinValues, const double *controlMaxValues, const double *controlMean, const double *controlPriorStd, const double *controlDiffPriorStd, const double *controlDiffDiffPriorStd, double controlMutationStdScale, const double *stateKernelStd, int nMORComponents)
	{
		...
		memcpy(&this->controlMin[0],controlMinValues,sizeof(double)*nControlDimensions);
		memcpy(&this->controlMax[0],controlMaxValues,sizeof(double)*nControlDimensions);
		if (controlMean!=NULL)
		{
			memcpy(&this->controlMean[0],controlMean,sizeof(double)*nControlDimensions);
		}
		else
		{
		...
		bestCost=DBL_MAX;  //not yet found any solution
		timeAdvanced=false;
		Eigen::VectorXd fullMin(nControlDimensions*nSteps),fullMax(nControlDimensions*nSteps);
		for (int step=0; step<nSteps; step++)
		{
	...
	void __stdcall ControlPBP::setSamplingParams( const double *controlPriorStd, const double *controlDiffPriorStd, const double *controlDiffDiffPriorStd, double controlMutationStdScale, const double *stateKernelStd )
	{
		if (stateKernelStd!=NULL)
		{
			memcpy(&this->stateKernelStd[0],stateKernelStd,sizeof(double)*nStateDimensions);
		...
		memcpy(&this->controlPriorStd[0],controlPriorStd,sizeof(double)*nControlDimensions);
		memcpy(&this->controlDiffPriorStd[0],controlDiffPriorStd,sizeof(double)*nControlDimensions);
		memcpy(&this->controlDiffDiffPriorStd[0],controlDiffDiffPriorStd,sizeof(double)*nControlDimensions);
	...
	void ControlPBP::finalizeWeights(int step )
	{
		...
		for (int i=0; i<nSamples; i++)
		{
			...
			v[i].controlPotential=exp(-0.5*v[i].controlCost);			//doesn't seem to be used, isn't initialized
		}
		...
		if (validDouble(maxUnnormalized / total))
		{
			for (int i=0; i<nSamples; i++)
			{
	...
	void ControlPBP::update( const double *currentState, TransitionFunction transitionFwd, StateCostFunction stateCostFunction, OnStepDone onStepDone )
	{
	...
	void __stdcall ControlPBP::startIteration(bool advanceTime, const double *initialState)
	{
		int nUniform=(int)(uniformBias*(double)nSamples);
		for (int i=0; i<nSamples; i++)
		{
			memcpy(&marginals[0][i].state[0],initialState,sizeof(double)*nStateDimensions);
			...
			marginals[0][i].bestStateCost=DBL_MAX;
	...
	double ControlPBP::getGBPRegularization()
	{
...
//				prior[step].weights[i]=marginals[step+1][i].weight;//*fwdMessage[step+1][i].weight;
	...
	void __stdcall ControlPBP::startPlanningStep( int step)
	{
		...
		if (bestCost<DBL_MAX && keepOldBest)  //bestCost<DBL_MAX means that the previous iteration found some path that is not totally useless
		{
			nInitialGuesses=1;
			if (useGaussianBackPropagation)
				nInitialGuesses++;
	...
	void __stdcall ControlPBP::updateResults( int sampleIdx, const double *finalControl, const double *newState, double squaredStateCost, const double *priorMean, const double *priorStd  )
	{
		MarginalSample &nextSample=marginals[nextStep][sampleIdx];

		//simulate and evaluate
		memcpy(&nextSample.state[0],newState,sizeof(double)*nStateDimensions);
		memcpy(&nextSample.control[0],finalControl,sizeof(double)*nControlDimensions);
		...
		{
			nextSample.bestStateCost=DBL_MAX;
		}
	...
	void __stdcall ControlPBP::getBestControl( int timeStep, double *out_control )
	{
		//the +1 because oldBest stores marginal states which store "incoming" controls instead of "outgoing"
		memcpy(out_control,oldBest[timeStep+1].control.data(),nControlDimensions*sizeof(double));
	}
	void __stdcall ControlPBP::getBestControlState( int timeStep, double *out_state )
	{
		//the +1 because oldBest stores marginal states which store "incoming" controls instead of "outgoing"
		memcpy(out_state,oldBest[timeStep+1].state.data(),nStateDimensions*sizeof(double));
	}
	...
	void __stdcall ControlPBP::setParams( double _uniformBias, double _resampleThreshold, bool _useGaussianBackPropagation, double _gbpRegularization )
	{
	...
	void __stdcall ControlPBP::getControl( int sampleIdx, double *out_control, const double *priorMean, const double *priorStd)
	{
	...
	void __stdcall ControlPBP::getControlWithoutStateKernel( int sampleIdx, double *out_control, const double *priorMean, const double *priorStd )
	{
		...
		else
		{
			...
			if (priorMean!=NULL && priorStd!=NULL)
			{
				memcpy(&prior->mean[0][0],priorMean,sizeof(double)*nControlDimensions);
				memcpy(&prior->std[0][0],priorStd,sizeof(double)*nControlDimensions);
				DiagonalGMM::multiply(*prior,*proposal,*proposal);	//ok to have same source and destination if both have only 1 gaussian
			}
